...
% Verify the influence of the distance to substrate on SBS gain
...
tintup_span = linspace(360, 460, 11);
SBSgain = zeros(length(tintup_span), length(freq_span));
overlap = zeros(length(tintup_span), length(freq_span));
...
tic;
for i = 1:length(tintup_span)
    tintup = tintup_span(i);
    model.param.set('t_intup', [num2str(tintup) ' [nm]']);
    model.param.set('freq_acous', [num2str(13.585) ' [GHz]']);
    model.study('std1').run();
    
    for j = 1:length(freq_span)
    freq_acous = freq_span(j);
    model.param.set('freq_acous', [num2str(freq_acous) ' [GHz]']);
    model.study('std2').run();
    SBSgain(i, j) = mphglobal(model,'SBSgain','dataset','dset4');
    overlap(i, j) = mphglobal(model,'overlapField','dataset','dset4');
    disp(['The distance to the x-axis is:' num2str(tintup_span(i)) ' nm'])
    disp(['SBSgain @' num2str(freq_acous) 'GHz is :' num2str(SBSgain(i, j))]);
    disp(['overlap @' num2str(freq_acous) 'GHz is :' num2str(overlap(i, j))]);
    toc;
    
    model.result.export('img1').set('sourceobject', 'pg2');
    model.result.export('img1').set('pngfilename', ['SDSautomationNo1changeSubsDistance\sweep(range360nm_460nm)\' num2str(tintup) 'nm\real\' num2str(tintup) 'nm' 'real(u_tZ) @' num2str(freq_acous) 'GHz']);
    model.result.export('img2').set('sourceobject', 'pg3');
    model.result.export('img2').set('pngfilename', ['SDSautomationNo1changeSubsDistance\sweep(range360nm_460nm)\' num2str(tintup) 'nm\imag\' num2str(tintup) 'nm' 'imag(u_tZ)@' num2str(freq_acous) 'GHz']);
