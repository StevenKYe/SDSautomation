...
grep all csv files in current directory with regular expression, plot the results, save the plot
@author: YeK
"""
import pandas as pd
import numpy as np
...
for file in fileList:
    ...
    data = pd.concat([data, dataSingle], axis=1)
    subplots.plot(dataSingle.iloc[:, 0],
                  dataSingle.iloc[:, 1], label=str(label))
